...
from database import (
    init_db, save_message, get_user_messages, get_users_by_status, set_conversation_status,
    get_user_info, save_auth_token, get_auth_token, get_conversation_status
)
...
""" def callback2():
    ...
    try:
        ...
        # Convert to long-lived token
        long_lived_token = exchange_short_for_long_token(short_lived_token['access_token'])
        if not long_lived_token:
            return render_template('error.html', message="Failed to get long-lived token")

        # Store tokens in session
        session['access_token'] = long_lived_token['access_token']
        session['token_expires'] = (datetime.now() + timedelta(days=60)).strftime('%Y-%m-%d %H:%M:%S')

        # Get Instagram business account ID
        account_info = get_instagram_account_info(long_lived_token['access_token'])
...
def get_users(status):
    ...
    return render_template('partials/user_list.html', users=users, status=status.lower())
...
def send_message():
    ...
    if result:
        # Save to database
        save_message(sender_id, message, is_from_me=True)
...
def set_assistant_mode():
    ...
    instagram.send_message(sender_id, notification)
    save_message(sender_id, notification, is_from_me=True)
            ...
            if 'entry' in data and len(data['entry']) > 0:
                ...
                if messaging:
                    ...
                    if sender_id and text:
                        ...
                        print(f"Stored token for {recipient_id}: {stored_token}, stored token type {type(stored_token)}")# access_token: {stored_token.get('access_token')}")
                        #print(f"Stored token for {recipient_id}: {stored_token}, access_token: {stored_token.get('access_token')}")
...
def save_manual_token():
    ...
    # Save to database
    save_auth_token(business_id, access_token)
